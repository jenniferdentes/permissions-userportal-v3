  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 px-3 py-2 bg-[var(--bg-default)] border border-[var(--divider)] rounded-lg text-sm text-[var(--fg-1)] hover:border-[var(--icon-base)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-600)] focus:ring-offset-1 transition-colors min-w-[130px]"
      >
        <span className="flex-1 text-left truncate">{selected?.label}</span>
        <IconChevronDown className="w-3.5 h-3.5 text-[var(--icon-subtle)] flex-shrink-0" />
      </button>
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[200px] bg-[var(--bg-default)] border border-[var(--divider)] rounded-lg shadow-lg py-1 max-h-56 overflow-y-auto">
          {QUICK_CHECK_ACTIONS.map((opt) => (
            <button
              key={opt.value}
              type="button"
              onClick={() => { onChange(opt.value); setOpen(false) }}
              className={`w-full text-left px-3 py-2 text-sm hover:bg-[var(--bg-paper-elev-1)] transition-colors ${
                opt.value === value ? 'text-[var(--fg-1)] font-medium bg-[var(--bg-paper-elev-1)]' : 'text-[var(--fg-2)]'
              }`}
            >
              {opt.label}
            </button>
...
function SiteSelect({ value, onChange, sitesData }: { value: string; onChange: (v: string) => void; sitesData: SiteData[] }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[180px] bg-[var(--bg-default)] border border-[var(--divider)] rounded-lg shadow-lg py-1">
          {sitesData.map((sd) => (
            <button
              key={sd.site.id}
              type="button"
              ...
              className={`w-full text-left px-3 py-2 text-sm hover:bg-[var(--bg-paper-elev-1)] transition-colors ${
                sd.site.id === value ? 'text-[var(--fg-1)] font-medium bg-[var(--bg-paper-elev-1)]' : 'text-[var(--fg-2)]'
              }`}
            >
              {sd.site.name}
            </button>
  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 px-3 py-2 bg-[var(--bg-default)] border border-[var(--divider)] rounded-lg text-sm text-[var(--fg-1)] hover:border-[var(--icon-base)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-600)] focus:ring-offset-1 transition-colors min-w-[180px]"
      >
        {selectedUser && (
          <div
      ...
      {open && (
        <div className="absolute z-50 mt-1 bg-[var(--bg-default)] border border-[var(--divider)] rounded-xl shadow-xl w-72 flex flex-col">
          {showPersonPicker && (
            <div className="flex gap-1.5 p-2.5 border-b border-[var(--divider)]">
              <button
                type="button"
                ...
                className={`flex-1 py-1.5 px-3 rounded-lg text-sm font-medium transition-colors ${
                  tab === 'name'
                    ...
                    : 'bg-[var(--bg-default)] border border-[var(--divider)] text-[var(--fg-2)] hover:bg-[var(--bg-paper-elev-1)]'
                }`}
              >
                By Name
              </button>
          ...
          <div className="px-3 py-2.5 border-b border-[var(--divider)]">
            <div className="flex items-center gap-2 px-2.5 py-1.5 border border-[var(--divider)] rounded-lg bg-[var(--bg-default)]">
          ...
          <div className="overflow-y-auto max-h-64 py-1">
            {tab === 'name' ? (
              filteredUsers.length === 0 ? (
                <div className="px-3 py-4 text-sm text-[var(--fg-disabled)] text-center">No results</div>
              ) : (
                filteredUsers.map((user) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[var(--bg-paper-elev-1)] transition-colors ${
                      selectedUser?.id === user.id ? 'bg-[var(--bg-paper-elev-1)]' : ''
                    }`}
                  >
                    <Avatar user={user} />
                    <div className="text-left min-w-0">
                      <div className="text-sm font-medium text-[var(--fg-1)] truncate">{user.name}</div>
                      <div className="text-xs text-[var(--fg-2)] truncate">{user.jobTitle}</div>
                    </div>
                  </button>
                ))
              )
              ...
              filteredTitles.length === 0 ? (
                ...
                filteredTitles.map((title) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm hover:bg-[var(--bg-paper-elev-1)] transition-colors ${
                      !selectedUser && jobTitle === title ? 'text-[var(--fg-1)] font-medium bg-[var(--bg-paper-elev-1)]' : 'text-[var(--fg-2)]'
                    }`}
                  >
                    {title}
                  </button>
    ...
    <div className="bg-[rgba(225,216,241,0.15)] dark:bg-[#9b58d6]/10 rounded-xl border border-dashed border-[#C4B2E5] dark:border-[#9b58d6]/30 px-6 py-6">
      <div className="mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-[var(--fg-1)]">Quick Check</h2>
        </div>
        <p className="text-sm text-[var(--fg-2)] mt-0.5 ml-6">Try a specific scenario before you act.</p>
      </div>
      ...
      <div className="hidden sm:flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="px-4 py-2 bg-[var(--brand-600)] text-white text-sm font-medium rounded-lg hover:opacity-90 active:opacity-80 disabled:opacity-40 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-[var(--brand-600)] focus:ring-offset-1 transition-opacity"
        >
          Check
        </button>
      </div>
      ...
      <div className="sm:hidden flex flex-col gap-2">
        <span className="text-base font-semibold text-[var(--fg-1)]">Can I</span>
